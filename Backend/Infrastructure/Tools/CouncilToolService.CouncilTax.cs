...
public partial class CouncilToolService
{
    ...
    // Scrape live Bradford council tax rates from bradford.gov.uk
    private async Task<Dictionary<string, (decimal Annual, decimal Monthly)>> ScrapeBradfordCouncilTaxRatesAsync(CancellationToken ct)
    {
        try
        {
            var html = await FetchHtmlAsync(
                "https://www.bradford.gov.uk/council-tax/council-tax-bands-and-rates/", ct);
            if (html == null) return new();

            var doc    = new HtmlDocument();
            doc.LoadHtml(html);
            var result = new Dictionary<string, (decimal Annual, decimal Monthly)>(StringComparer.OrdinalIgnoreCase);
            ...
            foreach (var table in tables)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    var col0 = CleanText(cells[0].InnerText).ToUpper();
                    if (col0.Length != 1 || col0[0] < 'A' || col0[0] > 'H') continue;

                    for (int i = 1; i < cells.Count; i++)
                    {
                        var raw = CleanText(cells[i].InnerText).Replace("£", "").Replace(",", "").Trim();
                        if (decimal.TryParse(raw, out var amount) && amount > 500 && amount < 10000)
                        {
                            result[col0] = (amount, Math.Round(amount / 12, 2));
                            break;
                        }
                    }
                }
        ...
        {
            _logger.LogWarning("Bradford rates scrape failed: {Msg}", ex.Message);
            return new();
        }
    }
}
