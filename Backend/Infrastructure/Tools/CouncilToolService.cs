...
public partial class CouncilToolService : IToolService
{
    ...
    public List<ToolDefinition> GetToolDefinitions() => new()
    {
        ...
        new ToolDefinition { Function = new FunctionDef { Name = "get_your_council_info", Description = "Bradford Council governance information. Call for: about Bradford Council, how the council works, chief executive, council constitution, political composition, which party controls the council, whistleblowing, Best Value Notice, Bradford Improvement Panel, council committee meetings, meeting minutes and agendas, your local councillor, find a councillor by ward, portfolio holders, cabinet members, council budgets and spending, council fees and charges, council buildings and offices, City Hall, Lord Mayor, inviting the Lord Mayor, ePetitions, sign a petition, report fraud, benefit fraud, housing fraud, types of fraud, counter-fraud policy, equality and diversity, scrutiny committees, parish councils, town councils, community right to challenge, Bradford District Partnership, budget consultation.", Parameters = new { type="object", properties=new{query=new{type="string",description="Council governance topic e.g. 'find my local councillor', 'attend a council meeting', 'report benefit fraud', 'invite the Lord Mayor'"}}, required=new[]{"query"} } } },
        new ToolDefinition { Function = new FunctionDef { Name = "get_births_deaths_marriages_info", Description = "Bradford Register Office information. Call for: register a birth, register a death, register a stillbirth, naming ceremonies, changing a child's name, get married in Bradford, civil partnerships, give notice of marriage, urgent marriages, convert civil partnership to marriage, renew vows, get a copy birth/death/marriage certificate, family history certificates, citizenship ceremonies, coroner's office, refer a death to the coroner, inquests, approved premises for weddings, register office fees.", Parameters = new { type="object", properties=new{query=new{type="string",description="Births, deaths or marriages topic e.g. 'register a birth', 'get married Bradford', 'copy death certificate', 'burial costs'"}}, required=new[]{"query"} } } },
        new ToolDefinition { Function = new FunctionDef { Name = "get_emergencies_info", Description = "Bradford emergency information. Call for: flooding, flood warnings, flood pack, protecting property from flooding, what to do in a flood, preparing for emergencies, emergency management, business continuity, planning a safe event, council service disruptions, bank holiday closure times, winter gritting emergency, dangerous weather.", Parameters = new { type="object", properties=new{query=new{type="string",description="Emergency topic e.g. 'flooding Bradford', 'what to do in a flood', 'prepare for emergency', 'bank holiday council hours'"}}, required=new[]{"query"} } } },
    ...
    public async Task<string> ExecuteAsync(string toolName, string argumentsJson, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            return toolName switch
            {
                ...
                "get_your_council_info"              => await GetYourCouncilInfoAsync(args.GetValueOrDefault("query", ""), ct),
                "get_births_deaths_marriages_info"   => await GetBirthsDeathsMarriagesInfoAsync(args.GetValueOrDefault("query", ""), ct),
                "get_emergencies_info"               => await GetEmergenciesInfoAsync(args.GetValueOrDefault("query", ""), ct),
                "get_regeneration_info"              => await GetRegenerationInfoAsync(args.GetValueOrDefault("query", ""), ct),
